...
tier_1_2_cities = ['Agra', 'Ahmedabad', 'Ajmer', 'Aligarh', 'Amravati', 'Amritsar', 'Asansol', 'Aurangabad', 'Bareilly', 'Belgaum',\
				 'Bengaluru', 'Bhavnagar', 'Durg Bhilai', 'Mumbai', 'Bhopal', 'Bhubaneswar', 'Bhubaneshwar', 'Bijapur', 'Bikaner', 'Bilaspur',\
				  ...
				  'Jamnagar', 'Jamshedpur', 'Jhansi', 'Jodhpur', 'Kakinada', 'Kannur', 'Kanpur', 'Karnal', 'Kochi', 'Kolhapur', 'Kolkata',\
				   ...
				   'Moradabad', 'Mumbai', 'Mysore', 'Nagpur', 'Nanded', 'Nashik', 'Nellore', 'Delhi NCR', 'Patna', 'Chennai',\
				    'Allahabad', 'Pune', 'Purulia', 'Raipur', 'Rajahmundry', 'Rajkot', 'Ranchi', 'Rourkela', 'Salem', 'Sangli',\
				     ...
				     'Ujjain','Virar','Tirunelveli','Vellore','Vijayawada','Visakhapatnam','Warangal', 'New Delhi', 'Delhi', 'Madras']
...
def Check_Location(loc, city_names= tier_1_2_city_names):  
	location = None
	if loc.lower() in city_names:
		if loc.lower() in set([x.lower() for x in ZomatoData['City'].values]):
			location = ZomatoData['City'].values[[x.lower() for x in ZomatoData['City'].values].index(loc.lower())]
	try:
		if location==None:
			return {'location_result': 'Not Found!', 'location_name': None}
		else:
			return {'location_result': "Location Found!", 'location_name': location}
	except:
		dispatcher.utter_message("Sorry, please enter a valid request!")
...
class ActionSearchRestaurants(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		results = ZomatoData[(ZomatoData.City==location)&(min_val<=ZomatoData['Average Cost for two'])&(ZomatoData['Average Cost for two']<=max_val)]
		df_display = results[results.Cuisines.apply(lambda x:cuisine.lower() in x.lower())]
		df_display = df_display.sort_values(by='Aggregate rating', axis=0, ascending=False)
		df_display = df_display.head(5)
		response = ''
		if len(df_display)!=0:
			for index , rec in df_display.iterrows():
				response = response + rec['Restaurant Name'] + " in " + rec['Locality'] + " " + "has been rated " + str(rec['Aggregate rating']) + "\n"
			dispatcher.utter_message("" + response)
...
class ActionSendEmail(Action):
	...
	def run(self,dispatcher, tracker, domain):
		try: 
			...
			results = ZomatoData[(ZomatoData.City==location)&(min_val<=ZomatoData['Average Cost for two'])&(ZomatoData['Average Cost for two']<=max_val)]
			df_display = results[results.Cuisines.apply(lambda x:cuisine.lower() in x.lower())]
			df_display = df_display[['Restaurant Name', 'Address', 'Average Cost for two', 'Aggregate rating']]
			df_display = df_display.sort_values(by='Aggregate rating', axis=0, ascending=False)
			...
			if len(df_display)!=0:
				...
				""".format(df_display.to_html(index=False))
				...
				message['Subject'] = "Hey Foodie!! Details of Restaurants in " + location + ' as per your custom preferences'
				...
				session.quit()
				dispatcher.utter_message('Email Sent : Acknowledged')
				return[AllSlotsReset()]
			else:
				dispatcher.utter_message("No records found! Email not sent.")
		except:
			dispatcher.utter_message("Sorry, the email sending failed abruptly!")
